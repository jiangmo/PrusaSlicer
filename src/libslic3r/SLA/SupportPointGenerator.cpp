...
void SupportPointGenerator::add_support_points(SupportPointGenerator::Structure &s, SupportPointGenerator::PointGrid3D &grid3d)
{
    ...
    if (s.islands_below.empty()) {
        ...
        uniformly_cover({ *s.polygon }, s, s.area * tp, grid3d, true, true);
        return;
    }
    ...
    if (! s.overhangs.empty()) {
        uniformly_cover(s.overhangs, s, s.overhangs_area * tp, grid3d, false, false);
    }
}
...
void SupportPointGenerator::uniformly_cover(const ExPolygons& islands, Structure& structure, float deficit, PointGrid3D &grid3d, bool is_new_island, bool boundary)
{
    //int num_of_points = std::max(1, (int)((island.area()*pow(SCALING_FACTOR, 2) * m_config.tear_pressure)/m_config.support_force));

    float support_force_deficit = deficit; //structure.support_force_deficit(m_config.tear_pressure());
...
//    assert(! poisson_samples.empty());
    ...
    for (const Vec2f &pt : poisson_samples) {
        m_output.emplace_back(float(pt(0)), float(pt(1)), structure.zlevel, m_config.head_diameter/2.f, is_new_island);
